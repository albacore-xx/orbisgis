...
public class MapEditorPlugIn extends ViewPlugIn implements WorkbenchFrame, IEditor {
	...
	public void initialize(PlugInContext context, Automaton automaton) {
		...
		if(context.getWorkbenchContext().getWorkbench().getFrame().getViewDecorator("Map")==null)
			context.getWorkbenchContext().getWorkbench().getFrame().getViews().
			add(new ViewDecorator(this,"Map",getIcon("map.png"),editors));		 
	}   
