...
import org.orbisgis.plugins.core.ui.extensions.tools.HelloTool;
...
public class OrbisConfiguration implements Setup {
	...
	//TODO:panel plugin with swing component to execute action on map editor
	...
	public void setup(WorkbenchContext workbenchContext) throws Exception {
		...
		//Record default tool
		Services.registerService(Automaton.class,"Gives default tool to the editor", defaultTool);
		...
		//TODO : why set TableEditor & MapEditor popup? Currently popup is build after right click event!  
		//Because there are several Map & Table editors so I record popup for all instances
	...
	private void configureTools(PlugInContext plugInContext)
	throws Exception {

		OrbisGISFrame frame = plugInContext.getWorkbenchContext()
			.getWorkbench().getFrame();
		
		// Main Toolbar & Main menu	-> sont chargés par le PlugIn lib/ext/MainToolBarPlugIn.jar	
		...
		fullExtentPlugIn.initialize(plugInContext); //after Zoom out for group ZoomIn/ZoomOut
		...
		add(new FencePolygonTool(), "shape_polygon_edit.png", frame.getFenceToolBar());
		...
		add(new EditionSelectionTool(), "select.png", frame.getEditionMapToolBar());
		add(new PointTool(), "point.png", frame.getEditionMapToolBar());
		add(new MultipointTool(), "multipoint.png", frame.getEditionMapToolBar());
		add(new LineTool(), "line.png", frame.getEditionMapToolBar());
		add(new MultilineTool(), "multiline.png", frame.getEditionMapToolBar());
		add(new MultipolygonTool(), "multipolygon.png", frame.getEditionMapToolBar());
		add(new VertexAditionTool(), "vertexadition.png", frame.getEditionMapToolBar());
		add(new VertexDeletionTool(), "vertexdeletion.png", frame.getEditionMapToolBar());		
	...
	private void configureToolBar(PlugInContext plugInContext) {
		...
		//Add Toolbars
		WorkbenchToolBar wbMain = new WorkbenchToolBar(wbContext,Names.TOOLBAR_NAME);
		wbToolBar.add(wbMain);
		WorkbenchToolBar wbNavigation = new WorkbenchToolBar(wbContext,Names.TOOLBAR_NAVIGATION);
		wbToolBar.add(wbNavigation);
		WorkbenchToolBar wbInfo = new WorkbenchToolBar(wbContext,Names.TOOLBAR_INFO);
		wbToolBar.add(wbInfo);
		WorkbenchToolBar wbFence = new WorkbenchToolBar(wbContext,Names.TOOLBAR_FENCE);
		wbToolBar.add(wbFence);
		WorkbenchToolBar wbMesureArea= new WorkbenchToolBar(wbContext,Names.TOOLBAR_MESURE);
		wbToolBar.add(wbMesureArea);		
		WorkbenchToolBar wbEditionMap= new WorkbenchToolBar(wbContext,Names.TOOLBAR_MAP);
		wbToolBar.add(wbEditionMap);
		WorkbenchToolBar wbEditionTable= new WorkbenchToolBar(wbContext,Names.TOOLBAR_TABLE);
		wbToolBar.add(wbEditionTable);
/*		WorkbenchToolBar wbMesureDistance= new WorkbenchToolBar(wbContext,"Mesure distance...");
	...
	private void configureMainMenus(final WorkbenchContext workbenchContext)
		...
		JMenu defaultMenu = new JMenu(Names.FILE);
		...
		JMenu helpMenu = (JMenu) FeatureInstaller.installMnemonic(
				new JMenu(Names.HELP), menuBar);
		menuBar.add(helpMenu);
		JMenu viewMenu = (JMenu) FeatureInstaller.installMnemonic(
				new JMenu(Names.VIEW), menuBar);
