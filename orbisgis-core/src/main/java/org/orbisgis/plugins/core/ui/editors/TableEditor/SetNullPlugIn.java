...
public class SetNullPlugIn extends AbstractPlugIn {
	...
	public boolean execute(PlugInContext context) throws Exception {
		...
		try {
			element.getDataSource().setFieldValue(
					((TableComponent)editor.getView().getComponent()).getTable().rowAtPoint(event.getPoint())
					, ((TableComponent)editor.getView().getComponent()).getTable().columnAtPoint(event.getPoint()),
	...
	public boolean isEnabled(Object arg) {return true;}		
	...
	public boolean isVisible(Object arg) {		
		...
		if("Table".equals(em.getEditorId(editor)) && editor!=null) {
			...
			TableEditableElement element = (TableEditableElement) editor.getElement();
			try {				
				return isVisible = element.isEditable()	&& !element.getDataSource().isNull(
						((TableComponent)editor.getView().getComponent()).getTable().rowAtPoint(event.getPoint()), 
						((TableComponent)editor.getView().getComponent()).getTable().columnAtPoint(event.getPoint()));
			} catch (DriverException e) {
				Services.getService(ErrorManager.class).error("Cannot set null row", e);
				return false;
			}
		}
		return isVisible = false;
