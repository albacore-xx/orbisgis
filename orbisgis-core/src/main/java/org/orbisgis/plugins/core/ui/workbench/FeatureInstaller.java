...
public class FeatureInstaller {
	...
	public static JMenu addMainMenu(FeatureInstaller featureInstaller,
			String[] menuPath, String menuName, int index) {
		JMenu menu = new JMenu(menuName);
		JMenu parent = featureInstaller.createMenusIfNecessary(featureInstaller
				.menuBarMenu(menuPath[0]), featureInstaller.behead(menuPath));
	...
	public JMenu createItemsIfNecessary(JMenu parent, String[] menuPath) {
		if (menuPath.length == 0) {
			return parent;
		}
		JMenu child = (JMenu) childMenuItem(menuPath[0], parent);
	...
	public JMenuItem addMainMenuItem(PlugIn executable,
			String[] menuPath, String menuItemName, boolean checkBox,
			ImageIcon icon, String[] editors, JComponent panel, WorkbenchContext wbContext) {
		//If PlugIn is a View PlugIn get is panel Component
		if (wbContext != null && panel!=null)			
			((ViewPlugIn)executable).createUpdatePlugInFactory(panel, menuItemName, icon, editors, wbContext);
		JMenuItem menuItem = installMenuItem(executable, menuPath, menuItemName, checkBox, icon);
		return menuItem;
	}
	...
	public JMenu addPopupMenuItem(WorkbenchFrame frame,
		...
		for(int i=0 ; i<menuPath.length; i++) {	
			...
			mymenu = new Menu(parent,menuPath[i],
					i!=menuPath.length-1?null:group,
					menuPath[i],
