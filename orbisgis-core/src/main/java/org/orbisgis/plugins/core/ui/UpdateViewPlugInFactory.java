...
public class UpdateViewPlugInFactory {
	...
	public UpdateViewPlugInFactory(WorkbenchContext workbenchContext, PlugIn plugIn) {
	...
	public void loadView(String id) {
		if (getViewDecorator(workbenchContext.getWorkbench().getFrame().getViews(),id).isOpen()) {
			getViewDecorator(workbenchContext.getWorkbench().getFrame().getViews(),id).close();				
		} else {
			EditorManager em = Services.getService(EditorManager.class);
			IEditor activeEditor = em.getActiveEditor();
			getViewDecorator(workbenchContext.getWorkbench().getFrame().getViews(),id).open(workbenchContext.getWorkbench().getFrame().getRoot(), activeEditor,
					em.getEditorId(activeEditor));							
		}
	}

	public boolean viewIsOpen(String id){
		return getViewDecorator(workbenchContext.getWorkbench().getFrame().getViews(),id).isOpen();
	}

	private ViewDecorator getViewDecorator(ArrayList<ViewDecorator> views, String id) {	
