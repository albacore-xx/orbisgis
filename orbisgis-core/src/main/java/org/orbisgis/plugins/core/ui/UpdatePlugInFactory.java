...
import org.orbisgis.core.ui.views.geocognition.GeocognitionTree;
...
public class UpdatePlugInFactory {
	...
	public boolean checkLayerAvailability() {	    
		MapContext mapContext = workbenchContext.getWorkbench().getFrame().getToc().getMapContext();
		if (mapContext != null) {
	...
	public boolean geocognitionIsVIsible(){
		GeocognitionTree tree = workbenchContext.getWorkbench().getFrame().getGeocognition().getTree();
	...
	public void executeGeocognition() {
		GeocognitionTree tree = workbenchContext.getWorkbench().getFrame().getGeocognition().getTree();
		Geocognition geocog = Services.getService(Geocognition.class);
		GeocognitionElement[] elements = GeocognitionTree.toElementArray(tree.getSelection());			
	...
	public void executeGeocognitionElement(INewGeocognitionElement element) {
		...
		NewGeocognitionObject[] objs = new NewGeocognitionObject[element.getElementCount()];
	...
	public boolean geocatalogIsVisible(){
		String[] res = workbenchContext.getWorkbench().getFrame().getGeocatalog().getSelectedSources();
		...
		if (plugin.acceptsSelectionCount(res.length)) {
			for (String source : res) {
				try {
					...
					acceptsAllSources = false;
					Services.getErrorManager().error("Error getting pop up : " + t);						
				}
			}
		} else {
			acceptsAllSources = false;
	...
	public static void checkTool(Automaton automaton) {
		...
		if(editor==null || !(editor instanceof MapEditorPlugIn))
			...
			.getObject();
			if (mapContext != null) {
				check = automaton.isEnabled(mapContext, ((MapControl)mapEditor.getComponent())
						.getToolManager());
				automaton.getButton().setEnabled(check);
				check = automaton.isVisible(mapContext, ((MapControl)mapEditor.getComponent())
						.getToolManager());
				automaton.getButton().setVisible(check);
				automaton.getButton().setSelected(((MapControl)mapEditor.getComponent()).getTool().getClass().equals(
						automaton.getClass()));
			}
		}
	}
