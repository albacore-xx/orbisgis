...
public class Catalog extends JPanel implements DragGestureListener,
	...
	private JPanel getFilterPanel() {
		...
		//TODO : mettre sous forme de plugins filters
/*		context.getFeatureInstaller().addFilter(frame, new All(), wbContext);
		...
		filter = new GeocatalogFilterDecorator(
				"geocatalog.filters.Files", "Files", new Files());
		listModel.getFilters().add(filter);
		
		filter = new GeocatalogFilterDecorator(
				"geocatalog.filters.DBs", "Databases", new DBs());
		listModel.getFilters().add(filter);
		
		filter = new GeocatalogFilterDecorator(
				"geocatalog.filters.Alphanumeric", "Alphanumeric", new Alphanumeric());
		listModel.getFilters().add(filter);
		
		filter = new GeocatalogFilterDecorator(
				"geocatalog.filters.WMS", "WMS", new WMS());
		listModel.getFilters().add(filter);
		
		filter = new GeocatalogFilterDecorator(
				"geocatalog.filters.Raster", "Raster", new Raster());
		listModel.getFilters().add(filter);
		
		lstFilters = new JList(listModel.getFilters().toArray(new GeocatalogFilterDecorator[0])/*getAvailableFilters()*/);
	...
	public JPopupMenu getPopup() {		
		...
		//TODO : A quoi servais le code ci-dessous?
		// Add tagging menus
